...
run_models <- function(jobs_list, mass, initial_mass, time, group_id) {
  ...
  output_list <- foreach(i = 1:length(jobs_list)) %dopar% {
    ...
    model <- jobs_list[[i]]$model
    iter <- jobs_list[[i]]$cv_cluster

    fit <- execute_stan(jobs_list[[i]],
                        mass = mass,
    ...
    pred_real <- data.frame(model = rep(model, nrow(mT_pred)),
                            ...
                            draw = mT_pred$draw,
                            mT_real = rep(jobs_list[[i]]$test$log_mean, each = 3000),
                            stringsAsFactors = FALSE)
