...
output_list <- foreach(i = 1:2) %dopar% { #length(jobs_list)
  output_i <- extract_output(jobs_list[[i]])
  return(output_i)
}
...
# then plot and save
lapply(1:length(output_list), function(x) {
  df <- pred[pred$model == x,]
  
  png(paste0('output/stan/eval_plots', x, '.png'))
  ...
  boxplot(draw ~ mT_real, df, 
          ylab = 'Posterior predicted distrbitions',
          xlab = 'Real test data')
  fit <- lm(mean_pred$mean ~ as.numeric(unique(df$mT_real)))
  abline(fit, col = 'red')
  legend('topleft', bty = 'n', legend = paste('R2 is', format(summary()$adj.r.squared, digits = 4)))
